...
export default function FriendsPage() {
  ...
  const handleAccept = async (requestId: number) => {
    await api.post(
      `/api/Friend/req/answer?requestId=${requestId}&state=Accepted`
    );
    loadAll();
  };
  ...
  const handleReject = async (requestId: number) => {
    await api.post(
      `/api/Friend/req/answer?requestId=${requestId}&state=Rejected`
    );
    loadAll();
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-2 border-b border-purple-200">
        ...
        {/* users tab removed - search moved to navbar */}
      </div>
          ...
          {friends.length === 0 ? (
            ...
            friends.map((f) => (
              ...
              >
                ...
                <button
                  className="btn"
                  onClick={() => handleUnfriend(getFriendId(f))}
                >
                  Remove
                </button>
              </div>
      ...
      {/* Users removed from this page - use navbar search */}
    </div>
  );
}
